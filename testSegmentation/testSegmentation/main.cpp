...
{
	Mat orgImg;
	orgImg=imread("../Dataset/Original_Dataset/0_1.png",CV_LOAD_IMAGE_ANYCOLOR);
	int i=0,j=1;
	while(i<10)
	{
		while(j<11)
		{
			...
			while(x<orgImg.cols)
			{
				orgImg.at<uchar>(orgImg.rows-1, x) = 0;
				orgImg.at<uchar>(orgImg.rows-2, x) = 0;
				orgImg.at<uchar>(orgImg.rows-3, x) = 0;
				orgImg.at<uchar>(orgImg.rows-4, x) = 0;
				orgImg.at<uchar>(orgImg.rows-5, x) = 0;
				x++;
			}
...
Mat averageFilter3x3(Mat img) {
	...
	int x,y;
	for(x=0;x<tmpImg.rows;x++)
		for(y=0;y<tmpImg.cols;y++)
			tmpImg.at<uchar>(x,y) = 0;
	
	for(x=1;x<=img.rows;x++)
		for (y=1;y<=img.cols;y++)
			tmpImg.at<uchar>(x,y) = img.at<uchar>(x-1,y-1);
	
	Mat avFilter3X3 = Mat::ones(3, 3, CV_8UC1);
	
	for(x=1;x<tmpImg.rows-1;x++)
		for(y=1;y<tmpImg.cols-1;y++)
			finalImg.at<uchar>(x-1,y-1) = mask5(avFilter3X3,tmpImg,y,x);
...
Mat averageFilter5x5(Mat img) {
	...
	for(x=2; x<=img.rows+1;x++)
		for (y=2;y<=img.cols+1;y++)
			tmpImg.at<uchar>(x,y) = img.at<uchar>(x-2,y-2);
	
	Mat avFilter5X5 = Mat::ones(5, 5, CV_8UC1);
	
	for(x=2;x<tmpImg.rows-2;x++)
		for(y=2;y<tmpImg.cols-2;y++)
			finalImg.at<uchar>(x-2,y-2) = mask5(avFilter5X5, tmpImg, y,x);
